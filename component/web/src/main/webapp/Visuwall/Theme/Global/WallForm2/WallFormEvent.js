define(
		[ 'jquery' ],
		function($) {
			var WallFormEvent = function(element, wallFormView) {
				var $this = this;
					...
					$this.tabs
							.tabs({
								...
								add : function(event, ui) {
									var contents = $('div[id^="tabs-"]',
											context);

									// -1 is the last but the last is the
									// current added one,
									// so its -2
									var newContent = $(
									...
									sliderInit(
											$(
													'DIV.projectFinderDelaySecondSlider',
													newContent),
											'projectFinderDelaySecond');
									$(
											'INPUT:regex(id,softwareAccesses.*\.projectFinderDelaySecond)',
											newContent)
											.change(
													function() {
														delayChange(this,
									...
									sliderInit(
											$(
													'DIV.projectStatusDelaySecondSlider',
													newContent),
											'projectStatusDelaySecond');
									$(
											'INPUT:regex(id,softwareAccesses.*\.projectStatusDelaySecond)',
											newContent)
											.change(
													function() {
														delayChange(this,
				...
				var urlFunc = function(urlObj, loginObj, passObj) {
					...
					var domObj = $('#' + urlObj.attr('id').replace(".", "\\.")
							+ "check", urlObj.parent());
									...
									function(softwareInfo) {
										...
										if (softwareInfo.softwareId.warnings) {
											...
											var ff = '				<table class="softwareInfo">'
													...
													+ '					<td>'
													+ softwareInfo.pluginInfo.name
													+ '</td>'
													+ '				</tr>'
													+ '				<tr>'
													+ '					<td class="label">version :</td>'
													+ '					<td>'
													+ softwareInfo.pluginInfo.version
													+ '</td>'
													+ '				</tr>'
													+ '			</table>'
													...
													+ '					<td class="label">name :</td>'
													+ '					<td>'
													+ softwareInfo.softwareId.name
													+ '</td>'
													+ '				</tr>'
													+ '				<tr>'
													+ '					<td class="label">version :</td>'
													+ '					<td>'
													+ softwareInfo.softwareId.version
													+ '</td>'
													+ '				</tr>'
													+ '				<tr>'
													+ '					<td colspan="2">'
													+ softwareInfo.softwareId.warnings
													+ '</td>'
													+ '				</tr>'
													+ '			</table>';
										...
										if ($
												.inArray(
														'build',
														softwareInfo.pluginInfo.capabilities) != -1) {
											$('FIELDSET.buildField', tabContent)
													.show();
										} else {
											$('FIELDSET.buildField', tabContent)
													.hide();
										...
										for ( var propertyName in softwareInfo.pluginInfo.properties) {
											...
											str += '<label for="' + preId
													+ 'properties-'
													+ propertyName + '">'
													+ propertyName.ucFirst()
													+ '</label>';
											str += '<input id="'
													+ preId
													+ 'properties-'
													+ propertyName
													+ '" class="ui-widget-content ui-corner-all" name="'
													+ preName + 'properties['
													+ propertyName
													+ ']" value="'
													+ propertyValue + '" />';
										...
										// project Names
										var projectNamesFormElem = $(
												'SELECT:regex(id,softwareAccesses.*\.projectNames)',
												tabContent);
										...
										// views
										var projectViewsFormElem = $(
												'SELECT:regex(id,softwareAccesses.*\.viewNames)',
												tabContent);
										var oldVal = projectViewsFormElem.val();
										if (oldVal == null) {
											oldVal = $(projectViewsFormElem)
													.data('newVal');
										}
				...
				this['INPUT:regex(id,softwareAccesses.*\.url)|change|live'] = function() {
					var login = $('INPUT:regex(id,softwareAccesses.*\.login)',
							$(this).parent());
					var password = $(
							'INPUT:regex(id,softwareAccesses.*\.password)', $(
									this).parent());
					urlFunc($(this), login, password);
				};

				this['INPUT:regex(id,softwareAccesses.*\.login)|change|live'] = function() {
					var url = $('INPUT:regex(id,softwareAccesses.*\.url)', $(
				...
				var sliderInit = function(context, id) {
					var func = function(event, ui) {
						$("SPAN." + id, $(context).parent()).html(
								ui.value + "s");
						$('INPUT:regex(id,softwareAccesses.*\.' + id + ')',
								$(context).parent()).val(ui.value);
					};
