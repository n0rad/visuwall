...
public class ProjectEnhancerService {
	...
	public void enhanceWithQualityAnalysis(Project analyzedProject, QualityConnectionPlugin qualityPlugin,
		...
		TestResult unitTestResultToMerge = build.getUnitTestResult();
		TestResult integrationTestResultToMerge = build.getIntegrationTestResult();

		if (qualityAnalysis != null) {
			mergeQualityAnalysis(qualityResultToMerge, qualityAnalysis);
		}
		if (unitTestsAnalysis != null) {
			mergeTestAnalysis(unitTestResultToMerge, unitTestsAnalysis);
		}
		if (integrationTestsAnalysis != null) {
