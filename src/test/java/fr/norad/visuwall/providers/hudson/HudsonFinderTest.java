...
import fr.norad.visuwall.providers.common.ClasspathFiles;
...
public class HudsonFinderTest {
    ...
    public void testFindCommiters() throws ResourceNotFoundException {
        ...
        Set<HudsonCommiter> commiters = hudsonFinder.findCommiters(new String[] { "username" });

        HudsonCommiter commiter = commiters.iterator().next();
        assertEquals("userId", commiter.getId());
        assertEquals("username", commiter.getName());
        assertEquals("myemail", commiter.getEmail());
    }
